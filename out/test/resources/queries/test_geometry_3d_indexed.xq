     ...
     try {
      ...
      return
       if (xs:boolean($vr/ggeo:isValid)) then
        ...
        for $message in $vr/ggeo:error/ggeo:msg
        return
         $message/text()
     } catch * {
      $err:description
  ...
  <geoIndexTest>
   {
    ...
    return
     if ($geom) then
      try {
       prof:void(ggeo:index($member, $geom))
      } catch * {
    ...
    return
     if (map:size($geometryParsingErrors) = 0) then
      'PASSED'
     else
      map:for-each($geometryParsingErrors, function ($a, $b) {
       'gmlid: ' || $a || ' - ' || $b
      })
