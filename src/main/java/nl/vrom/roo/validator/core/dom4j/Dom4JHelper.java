...
public final class Dom4JHelper {
	...
	public static void read(Reader reader, final ElementHandler... elementHandlers) throws DocumentException {
		if (elementHandlers == null || elementHandlers.length == 0) {
			throw new IllegalStateException("No elementhandlers defined for Dom4JValidation");
		}
	...
	public static org.w3c.dom.Element toW3CElement(org.dom4j.Element d4element) throws DocumentException {
	...
	public static String getNodeFromPath(String path) { // NOPMD Suppress PMD bug, this is not an empty method
		return path.substring(path.lastIndexOf('/') + 1).trim();
	}

	public static String getParentPath(String path) {// NOPMD Suppress PMD bug, this is not an empty method
